...
def learning_rock_paper_scissors(learning_data, learn_during):
    ...
    # 最後の要素まで含めるために len(learning_data) に修正
...
def judge_correct(test_raw_hands, learn_during, learn_hash):
    win_count = 0
    # 最後の要素まで含める
...
def main():
    ...
    # 過去3回分の手を元に次を予測する (3などの小さな整数を指定する)
    learn_during = 7
    learning_result = learning_rock_paper_scissors(train_hands, learn_during)
    
    win_rate = judge_correct(test_hands, learn_during, learning_result)
